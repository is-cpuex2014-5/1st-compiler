...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  ...
  | Sla(x, y') -> (Ans(Sla(find x Type.Int regenv, find' y' regenv)), regenv)
  | Srl(x, y') -> (Ans(Srl(find x Type.Int regenv, find' y' regenv)), regenv)
  | Sra(x, y') -> (Ans(Sra(find x Type.Int regenv, find' y' regenv)), regenv)
  | Load(x, y, z') -> (Ans(Load(find x Type.Int regenv, find y Type.Int regenv, find' z' regenv)), regenv)
  ...
  | FLoad(x, y, z') -> (Ans(FLoad(find x Type.Float regenv, find y Type.Int regenv, find' z' regenv)), regenv)
