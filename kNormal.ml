...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.Var(x) -> (* 外部配列及び外部タプルの参照 (caml2html: knormal_extarray) *)
      ...
      | t' -> failwith (Printf.sprintf "external variable %s does not have an array type but type %s" x (Type.string_of_type t')))
  ...
  let rec f n e' = 
    let rec g n = function
      ...
      | FDiv (s1, s2) -> Printf.fprintf oc "FDIV\n";
			 pid oc (n+1) s1; pid oc (n+1) s2
      | Itof(s) -> Printf.fprintf oc "ITOF\n"; 
		 pid oc (n+1) s
      | Ftoi(s) -> Printf.fprintf oc "FTOI\n"; 
		 pid oc (n+1) s
      | IfEq (s1, s2, e1, e2) -> Printf.fprintf oc "IFEQ\n";
				 pid oc (n+1) s1; pid oc (n+1) s2;
				 f (n+1) e1; f (n+1) e2
      | IfLT (s1, s2, e1, e2) -> Printf.fprintf oc "IFLT\n";
				 pid oc (n+1) s1; pid oc (n+1) s2;
				 f (n+1) e1; f (n+1) e2
